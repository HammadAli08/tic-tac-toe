...
describe('todos API', () => {
  ...
  it('returns 404 for an unknown id', async () => { expect((await request(app).patch('/todos/999')).status).toBe(404); expect((await request(app).delete('/todos/999')).status).toBe(404); });
});
